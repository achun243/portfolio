var filepath_pagesJSON = '/kr/sustainability/cmn/json/pages.json';
var filepath_tagTextJSON = '/kr/sustainability/cmn/json/tags.json';
...
function setTagHTML(json) {
  ...
  for (var i=0; i < conuntOfTags; i++) {
    var content = '<li class="tagSearch_listItem"><a class="tagSearch_listItemLink track_click ga_cat-sustainability ga_act-keyword-' + json[i]["query"] + '" href="/kr/sustainability/keyword/index.html?tag=' + json[i]["query"] + '"># ' + json[i]["tag_name"] + '</a></li>';
    html.push(content)
  }
  /*以下、「もっと見る」「閉じる」ボタンで隠す場合*/
  /*
  for (var i=0; i < conuntOfTags; i++) {
    var content = '<li class="tagSearch_listItem is-hidden"><a class="tagSearch_listItemLink" href="/kr/sustainability/keyword/index.html?tag=' + json[i]["query"] + '"># ' + json[i]["tag_name"] + '</a></li>';
    html.push(content)
  }*/
