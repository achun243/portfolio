    ...
    $("div#movie_modal_content").animate({
      ...
      complete: function () {
        ...
        if (!((ua.indexOf('iPhone') > 0 && ua.indexOf('iPad') === -1) || (ua.indexOf('Android') > 0 && ua.indexOf('Mobile Safari') > 0) || ua.indexOf('iPod') > 0 || ua.indexOf('BlackBerry') > 0)) {
          console.log("PC："+ua);
            $("#modalyoutube_player").html('<video autoplay muted playsinline width="960" height="540" src="' + href + '" loop controls></video>');
        } else {
          console.log("SP："+ua);
            $("#modalyoutube_player").html('<video muted playsinline width="300" height="169" src="' + href + '" loop controls></video>');
        }
      }
    });
  }
